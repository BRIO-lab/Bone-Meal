...
DATA_DIR="${DATA_ROOT}/dogs"
SAVE_DIR="${DATA_ROOT}/seg_result/dogs/"
BACKBONE="hrnet48"

CONFIGS="configs/jtml/H_48_D_4.json"
...
if [ "$1"x == "train"x ]; then
  ...
  ${PYTHON} -u main.py --configs ${CONFIGS} \
                       ...
                       --gpu 0 1 2 3 \
                       --resume_continue y \
                       --resume ./checkpoints/dogs/${CHECKPOINTS_NAME}_latest.pth \
                       --checkpoints_name ${CHECKPOINTS_NAME} \
                        2>&1 | tee -a ${LOG_FILE}
  ...
  if [ "$3"x == "ss"x ]; then
    ...
    ${PYTHON} -u main.py --configs ${CONFIGS} --drop_last y \
                         ...
                         --phase test --gpu 0 1 2 3 --resume ./checkpoints/dogs/${CHECKPOINTS_NAME}_latest.pth \
    ...
    ${PYTHON} -u main.py --configs ${CONFIGS_TEST} --drop_last y \
                         --backbone ${BACKBONE} --model_name ${MODEL_NAME} --checkpoints_name ${CHECKPOINTS_NAME} \
                         --phase test --gpu 0 1 2 3 --resume ./checkpoints/dogs/latest.pth \
                         --test_dir ${DATA_DIR}/test --log_to_file n \
