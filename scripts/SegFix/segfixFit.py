...
from scripts.lib.models.nets.segfix import SegFix_HRNet
from scripts.lib.models.loss.loss import SegFixLoss
"""
...
def main(config, wandb_run):
    ...
    segfix_model = SegFix_HRNet(config, backbone=model)
    ...
    #training
    #Decide if to use pytorch lightning or just train locally since we're just trying to make it work
    import cv2
    import numpy as np
    # Do something with map_mask and dir_mask that draw the boundaries on the objects.

    #Below code is to visualize input image vs map mask vs dir mask
    input_image = cv2.imread('input_image.jpg')
    mask_map = cv2.imread('mask_map.jpg', cv2.IMREAD_GRAYSCALE)
    dir_map = cv2.imread('dir_map.jpg', cv2.IMREAD_GRAYSCALE)
    mask_color_map = cv2.applyColorMap(mask_map, cv2.COLORMAP_JET)
    # Combine the input image, mask_color_map and dir_map into a single image for visualization
    output_image = np.zeros((input_image.shape[0], input_image.shape[1] * 3, 3), dtype=np.uint8)
    output_image[:, :input_image.shape[1], :] = input_image
    output_image[:, input_image.shape[1]:input_image.shape[1] * 2, :] = mask_color_map
    output_image[:, input_image.shape[1] * 2:, :] = cv2.cvtColor(dir_map, cv2.COLOR_GRAY2BGR)
